...
FROM reading_literacy.text_samples ts
JOIN reading_literacy.text_types tt ON ts.text_type_id = tt.type_id
WHERE ts.grade_id = 4
...
LEFT JOIN assessment.questions q ON ts.sample_id = q.text_sample_id
WHERE ts.grade_id = 4
GROUP BY ts.sample_id, ts.title_az, ts.word_count, tt.type_name_az, ts.created_at
...
SELECT 
    ...
    ts.grade_id,
    tt.type_name_az as text_type,
    tt.type_name_en as text_type_en,
    ts.source,
    ts.notes,
    ts.created_at,
...
GROUP BY ts.sample_id, ts.title_az, ts.title_en, ts.word_count, ts.grade_id,
         tt.type_name_az, tt.type_name_en, ts.source, ts.notes, 
